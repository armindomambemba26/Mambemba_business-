...
Storage), continuando sem servidor próprio.
...
- ✅ **Fotos dos produtos** ficam no **Firebase Storage**, também acessíveis em qualquer
...
ficheiro `firebase-config.js`. Falta ativar 3 serviços e aplicar as regras de segurança.
...
### 3.3 Ativar Storage
1. **Build → Storage → Get started**, modo produção.
2. Vá à aba **Rules**, apague o conteúdo e cole o ficheiro `storage.rules` deste
   pacote → **Publish**.

### 3.4 Criar a primeira conta Admin (manual, só na primeira vez)
...
storage.rules      → regras de segurança do Storage (colar na consola Firebase)
...
(Authentication + Firestore + Storage), sem precisar de um servidor próprio.
Ideias para continuar a evoluir, quando fizer sentido para o negócio:
