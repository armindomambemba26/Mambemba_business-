...
(function () {
  ...
  auth.onAuthStateChanged(function (fbUser) {
    ...
    teamColRef.doc(fbUser.uid).get().then(function (docSnap) {
      ...
      showLoginError("Não foi possível validar a conta (" + (err.code || "erro") + "): " + (err.message || "desconhecido"));
      auth.signOut();
    });
  });
  ...
  // Nota: a partir da v3.3 (mantido na v3.4) os dados vivem no Firebase (Firestore),
