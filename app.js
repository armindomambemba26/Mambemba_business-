...
(function () {
  ...
  function savePhoto(orderId, dataUrl) {
    return photosColRef.doc(orderId).set({ data: dataUrl }).catch(function (e) {
      console.error("Erro ao guardar foto:", e);
      toast("Não foi possível guardar a foto.", "error");
    });
  }
  var photoUrlCache = {}; // orderId -> dataURL (evita pedidos repetidos)
  function getPhoto(orderId) {
    ...
    return photosColRef.doc(orderId).get().then(function (docSnap) {
      var url = docSnap.exists ? docSnap.data().data : null;
  ...
  var photoCache = {}; // orderId -> dataURL pendente (pré-visualização antes de guardar)
  ...
  // Redimensiona e comprime a foto para caber num documento Firestore
  // (limite de 1MB por documento). Mantém qualidade suficiente para o recibo.
  ...
  $("#f_foto").addEventListener("change", function (e) {
    ...
    if (file.size > 8 * 1024 * 1024) {
      toast("Imagem muito grande. Escolha uma foto até 8MB.", "error");
      e.target.value = "";
      return;
    }
    compressImage(file, 900, 0.7).then(function (dataUrl) {
      // Se ainda ficar grande (fotos muito detalhadas), comprime mais uma vez.
      if (dataUrl.length > 700 * 1024) {
        return compressImage(file, 700, 0.55);
      }
      return dataUrl;
