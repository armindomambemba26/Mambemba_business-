...
(function () {
  ...
  function saveReceiptSummary(order) {
    ...
    receiptsColRef.doc(order.id).set({
    ...
    }).catch(function (e) { console.error("Erro ao guardar resumo do recibo:", e); });
  }
  ...
  function backfillReceiptSummaries() {
    // Cria o resumo público em falta para encomendas criadas antes desta
    // funcionalidade existir. Corre só uma vez por sessão, e só quando há
    // alguém com sessão iniciada (a escrita exige autenticação).
    ...
    receiptsColRef.get().then(function (snap) {
      ...
      DB.orders.forEach(function (o) {
        if (!existing[o.id]) saveReceiptSummary(o);
      });
    }).catch(function (err) { console.error("Erro ao preencher resumos de recibo:", err); });
  }
