...
(function () {
  ...
  function loadData() {
    try {
      ...
      if (!parsed.settings) parsed.settings = clone(DEFAULT_DATA.settings);
  ...
  DB = loadData();
  if (window.MBFirebase) {
    window.MBFirebase.pull().then(function(cloud){
      if (!cloud) { window.MBFirebase.push(DB); return; }
      DB = Object.assign(clone(DEFAULT_DATA), cloud);
      if (!DB.team || !DB.team.length) DB.team = clone(DEFAULT_DATA.team);
      saveData(DB);
      applyBranding();
  ...
  function persist() {
    saveData(DB);
    if (window.MBFirebase) window.MBFirebase.push(DB).catch(function(e){ console.error(e); });
  }
  ...
  loginForm.addEventListener("submit", async function (e) {
    ...
    if (!u || !p) { showLoginError("Preencha o utilizador e a palavra-passe."); return; }
    var match = DB.team.find(function (t) { return t.user && t.user.toLowerCase() === u.toLowerCase(); });
    if (match && match.estado === "Inativo") { showLoginError("Este utilizador está inativo. Contacte o administrador."); return; }
    try {
      if (window.MBFirebase) {
        await window.MBFirebase.signIn(u,p);
        if (u.toLowerCase()==='admin') await window.MBFirebase.ensureAdminProfile();
        var profile = await window.MBFirebase.getMyProfile();
        if (profile) match = { id: profile.id || profile.uid, user: profile.user || u, nome: profile.nome || u, funcao: profile.role || 'Vendedor', estado: profile.estado || 'Ativo' };
      } else {
        if (!match || match.pass !== p) throw new Error("invalid");
      }
      if (!match) { showLoginError("Utilizador não encontrado. Peça ao administrador para criar a sua conta."); return; }
      setSession({ id: match.id, user: match.user, nome: match.nome, role: match.funcao, ts: Date.now() });
      loginForm.reset(); enterApp();
    } catch (err) {
      console.error(err);
  ...
  orderForm.addEventListener("submit", function (e) {
    ...
    if (pendingPhotoDataUrl) {
      ...
      if (window.MBFirebase) {
        window.MBFirebase.uploadPhoto(id, pendingPhotoDataUrl).then(function(url){
          if(url){ var found=DB.orders.find(function(x){return x.id===id;}); if(found){ found.fotoUrl=url; persist(); } }
        });
      }
    }
  ...
  userForm.addEventListener("submit", function (e) {
    ...
    persist();
    if (window.MBFirebase && isNew && passVal) {
      window.MBFirebase.ensureProfile(member, passVal).then(function(r){
        if(r && r.error) toast("Utilizador guardado localmente, mas não foi possível criar a conta Firebase.", "error");
      });
    } else if (window.MBFirebase && !isNew) {
      window.MBFirebase.updateProfile(member);
    }
  ...
  $("#exportDataBtn").addEventListener("click", function () {
    var photoJobs = DB.orders.map(function (o) {
      return getPhoto(o.id).then(function (dataUrl) { return { id: o.id, dataUrl: dataUrl }; });
    });
  ...
  $("#importDataInput").addEventListener("change", function (e) {
    ...
    reader.onload = function () {
      ...
      confirmDialog("Importar este ficheiro vai substituir TODOS os dados atuais guardados neste dispositivo. Continuar?").then(function (ok) {
        e.target.value = "";
        if (!ok) return;
        DB = imported.data;
        if (!DB.settings) DB.settings = clone(DEFAULT_DATA.settings);
        if (!DB.team || !DB.team.length) DB.team = clone(DEFAULT_DATA.team);
        if (!DB.clients) DB.clients = [];
        if (typeof DB.seq !== "number") DB.seq = DB.orders.length;
        persist();
        var photoWrites = (imported.photos || []).map(function (p) {
          photoCache[p.id] = p.dataUrl;
  ...
  $("#resetDataBtn").addEventListener("click", function () {
    confirmDialog("Isto vai apagar PERMANENTEMENTE todas as encomendas, clientes e fotos guardadas neste dispositivo. A equipa e as definições da loja são mantidas. Continuar?").then(function (ok) {
      if (!ok) return;
      DB.orders = [];
      DB.clients = [];
      DB.seq = 0;
