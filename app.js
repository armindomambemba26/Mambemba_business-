...
(function () {
  ...
  function savePhoto(orderId, dataUrls) {
    ...
    return photosColRef.doc(orderId).set({ images: images }).catch(function (e) {
      console.error("Erro ao guardar fotos:", e);
      toast("Não foi possível guardar as fotos.", "error");
    });
  }
  ...
  function processOnePhoto(file) {
    return compressImage(file, 700, 0.6).then(function (dataUrl) {
      if (dataUrl.length > 150 * 1024) return compressImage(file, 500, 0.45);
      return dataUrl;
    });
  }
  ...
  orderForm.addEventListener("submit", function (e) {
    ...
    if (pendingPhotoDataUrls.length) {
      ...
      savePhoto(id, pendingPhotoDataUrls);
    }
